...
- **Wearables** (Asset System + Graphics Engine) — assets authored once against the template body that dynamically fit any body variant (they receive the body's variant transformations), mask the skin they cover to prevent clipping, and animate through the same skinning path. Slots hold layered stacks (base/mid/outer) with thickness-aware fitting and outward-cascading masking. Hairstyles are wearables. Held items attach rigidly via grip points, with grip types, dual wield, and sheathing. Wearables and held items are virtual-model compatible (P5).
- **Basic AI v1** — a data-defined state machine (idle/wander/patrol/chase/attack/flee/return) behind `AIController`, driven by perception and factions, LOD-scheduled per streaming residency; internals designed to be replaced by a richer behavior model later without changing the attachment contract. Basic facial expressions (a built-in blendable morph-preset set) ride the face variant machinery.
