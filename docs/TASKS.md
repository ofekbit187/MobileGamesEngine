...
- [~] **8.10** Skinned mesh rendering + skeleton runtime (pose evaluation, GPU skinning) within Phase 2 budgets — *17-joint canonical rig + `evaluatePose` live; the template body is now a real skinned mesh (36-byte `SkinVertex`, ≤4 influences) with a per-character 17-joint palette and `skinMesh()` as the CPU reference; the GPU skinning pipeline (palette upload + skinned vertex shader) is the remaining half — tools skin on the CPU at load time meanwhile*
