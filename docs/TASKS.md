...
- [~] **16.4** Reweight the shoulder (ADR 0015) — *built and MEASURED with the animation session's own strain gate, **not landed**. Reweighting more than halves the tear (worst edge strain 3.513 -> 1.578 at 140 deg; the 1.00-weight cliff goes 53 vertices -> 5; at 30 deg, locomotion's range, edges over 50% go 5 -> 0 and the layering test's locomotion worst drops 0.311 -> 0.265). It does **not** reach the acceptance number of zero edges over 100% at 140 deg — it reaches 18. Held rather than landed for three reasons, the third of which needs a ruling: it misses its bar; it introduces degenerate triangles; and **changing skin weights silently re-cuts the masking regions**, because `regionOf()` reads the dominant bone — which surfaced as the wearables `jaw` pit assertion failing on a body whose head did not change shape. Search space, dense-mesh control and the clavicle arithmetic in `docs/research/shoulder-reweight.md`; seam request in `docs/status/character-asset-pipeline.md`*
