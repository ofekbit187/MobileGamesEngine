...
- [ ] **16.4** **Reweight EVERY bending joint** (ADR 0015 + ADR 0016) — **owner priority: first, above everything.** Root cause is not a badly weighted shoulder, it is that *nobody weighted it*: the pipeline binds with Blender's `ARMATURE_AUTO` bone heat and shipped the automatic result as a finished one. `B-15` already required 50/50 joint-loop weighting and `B-16` joint-extreme integrity — **the body violates both MUSTs** and has since import. Scope is every bending joint (elbows, knees, hips, neck, wrists came from the same bind and have never been posed either), not just the shoulder the archetypes happened to expose. **Acceptance: `B-31` — zero edges over 100% strain at each joint's working range**, >50% count reported, measured with the animation session's committed instrument. Contract-version event. Preempts everything — *owner: character asset pipeline*
- [ ] **16.6** Promote the per-edge strain instrument into the body contract's gate set as **§9.8** and label `ARMATURE_AUTO` at the call site as a draft that must pass `B-31` — the systemic half of ADR 0016, so the next imported body cannot repeat this — *owner: character asset pipeline*
...
- [ ] **16.5** `skinMesh()` writes `Vertex::uv` (ADR 0015) — it writes position and normal and leaves UV at `{0,0}`, so a CPU-skinned body samples one texel for its whole surface. **Blocks any textured character**, therefore blocks the first face texture, therefore blocks ADR 0012's Face-waiver retirement. One line — *owner: character asset pipeline*
