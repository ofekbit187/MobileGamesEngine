...
- [ ] **14.7** Held items on the real body — **no character in the shipped engine has ever held anything.** `buildPosedCharacter` and `device_game.cpp` both skip held items; the sword in earlier captures was the dead box rig generating a rigid part, not a feature that regressed. Phase 14's proof catalogue is bare-handed for this reason — *owner: wearables (§6.3)*
