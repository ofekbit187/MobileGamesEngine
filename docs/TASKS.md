...
- **PARTLY UNBLOCKED (ADR 0010/0011) — the chart is repacked; two rules still refuse.** `mge_uv_report --gate` now passes five of its seven checkable rules; `chart_regions_required` (Face) is task 13.7, and `stretch_above_max` is the CC0 source's own inherited unwrap distortion, whose decision ADR 0011 defers to 13.7's completion with a named trigger. No skin texture is authored yet. *Historical, for the record:* `mge_uv_report`
