...
- [~] **8.19** Held items: grip types (one-handed/two-handed/versatile), dual wield via `held_main`+`held_off`, item-defined grip points, drawn/sheathed states with sheath attachment points and default draw/sheath animations — *RE-CHECKED against the real skinned body (task 14.7), and the earlier claim was measured against the v1 box rig rather than the shipping path. What actually survived: nothing visual — `buildPosedCharacter` skipped held items entirely, so no character on the skinned path had ever rendered one. What was real and stayed real: the slot machinery and the sheathed FLAG (`equipment_slots_swap_and_sheathe`, `ai_draws_on_contact_and_sheathes_after`) — the AI genuinely draws and sheathes, it just had nothing to show for it. NOW DONE via 14.7: item-defined grip points, sheath attachment points, drawn/sheathed placement on the real body, grip types declared in data. STILL OPEN: dual wield (slot occupancy is gameplay's) and draw/sheath transition animations (the archetype library's `draw`)*
...
- [x] **13.12** Zero-code-per-garment proof: adding a wearable touches no `.cpp` and no `CMakeLists` — the build demonstrates it (P12) — *a wearable is now a `.mgewear` file next to its mesh, in the `key value` / `#` comment convention `assets/standards/skin_texture.mgestd` established. `WearableCatalogue` reads the asset directory; the three `switch` statements that defined what a garment could be (`garmentAsset`, `garmentCoverage`, and the bake tool's hardcoded array) are gone, replaced by lookups. The proof is `a_garment_added_as_pure_data_behaves_like_a_compiled_one`: it writes a surcoat `.mgewear` into a scratch directory at runtime — no enumerator, no `switch` arm, no rebuild — and requires it to mask, resolve its mesh and find its baked binding exactly like the shipped six. `WearableKind` DELIBERATELY SURVIVES as the name of the first six catalogue rows: removing it would touch Platform- and pipeline-owned files (`device_game.cpp`, `body_preview`, `test_body_mesh.cpp`), which is a seam change, and it is not needed for the P12 claim — a NEW garment needs no enumerator. Full removal is proposed in the ledger for the architect to rule on. A malformed `.mgewear` is refused by name, line number and reason, and does not take the rest of the catalogue down with it*
...
- [x] **14.3** Phase-addressable timeline: wind-up / strike / recovery exposed as fractions, and the Phase 12 action model's damage moment hung on `strike` — *the animation half was done first: `UsePhases` exposes the three as fractions plus a duration, `UsePlayer::strikeMoment()` gives the damage instant in seconds, and `update()` returns true on the single frame it is crossed. The same `swing` lands at 0.233 s for a dagger and 0.729 s for a maul, neither number tuned.*
  **Closed by ADR 0021, on the owner's instruction.** The animation session's warning was correct
  and saved the work from starting in the wrong place: there was **no tuned delay to delete**.
  `performUseHeld` applied damage inline on the input frame, `useCooldown` is a rate limiter and
  not a damage timer, and no deferral existed anywhere — so this was not a deletion but the
  introduction of one. It also correctly identified that ADR 0017's grant did not stretch this
  far, and raised it instead of taking it. That is the roster working.
  The obstacle it named is real: the strike moment is a property of the motion timeline in
  `character/`, and `framework/` does not include `character/`. Resolved by **injection**, the
  pattern `CharacterSystem` already uses for the item registry, collision and interactions:
  gameplay declares `StrikeDelayFn`, the character pillar supplies `strikeDelaySeconds`, the game
  wires them. With no timing installed, damage resolves immediately exactly as before, so nothing
  that predates this changed behaviour. Target is now chosen **at the strike**, so a victim who
  steps out of reach during the wind-up is missed and one who steps in is hit
- [x] **14.4** Interruption: hit/stagger/death blend out mid-action, never snap — *`UsePlayer::interrupt(seconds)` ramps the layer weight out; the motion keeps travelling underneath the fade so the limb carries on along its arc rather than freezing, and an interrupted action never reports its strike. Measured across three identical runs: worst frame-to-frame pose jump 0.650 rad uninterrupted, 0.549 blended, 2.172 snapped — blending adds NO discontinuity beyond the motion's own speed while snapping adds 3.3x. Now also proven END TO END through the real gameplay API rather than a stand-in: `a_real_hit_mid_action_blends_the_action_out` takes a real `CharacterSystem::damage()` mid-chop and blends out. **No `character.h` change was needed** — per-character animation state already lives on the game's actor struct beside its `LocomotionAnimator`, which is exactly where a `UsePlayer` belongs. One wart, named rather than hidden: `CharacterSystem` has no hit callback or damaged-this-tick flag, so a game notices by watching its own `health`. Polling works and is what the test does; an event would be gameplay's to add*
- [x] **14.5** Item data: archetype + reach + weight on `ItemUse`; `animKey` narrows to the bespoke-clip escape hatch — *ruled and assigned by ADR 0017. `engine/include/mge/framework/use_archetype.h` (new, gameplay's) holds `UseArchetype` and `ItemGrip`; `character/use_archetypes.h` includes it. The include direction was the architect's call and (a) was impossible — `character/` includes `framework/` one way, so putting the animation header inside `items.h` would have closed a directory cycle. `ItemUse` gains four defaulted fields, so nothing that existed before this changed behaviour. `motionFromItemUse()` is the bridge and CLAMPS on the way across — item data is content. `usesBespokeClip()` is the `animKey` test, and a hero item still carries a usable archetype underneath so a missing clip falls back to animating the kind rather than standing still. Tests: `an_item_declares_its_use_and_the_engine_animates_it`, `an_item_that_says_nothing_about_its_use_still_animates`, `animKey_narrows_to_the_bespoke_clip_escape_hatch`. No save-schema bump — the architect grepped it and `ItemUse` is not serialized*
- [x] **14.6** Proof by catalog: sword, spear, axe, hammer, torch, apple — six items, zero per-item animation authoring, visibly distinct motion — *now true through real item data rather than a demo struct: the six are plain `ItemUse` literals registered in an ordinary `ItemUseRegistry` by asset id, and `an_item_declares_its_use_and_the_engine_animates_it` takes them back out and measures the motions pairwise distinct. Zero `.cpp` and zero `CMakeLists` lines per item; a seventh item is four numbers. Closest pair 82.2 degrees apart across their timelines. Capture `anim_catalog.ppm`, bare-handed because held items are not drawn on the real body anywhere (14.7). **One follow-up that is not mine:** the app's own catalogue in `device_game.cpp` (Platform's) still declares sword/apple/torch without archetypes, so they take the `Swing` default — the apple would swing. Harmless today because nothing drives a `UsePlayer` yet, and it is a two-line data change whenever Platform next touches that file*
...
- [x] **16.2** `tools/anim_preview` onto the real skinned body + 14.1 re-verified through skinning — *`6276768`. Verified with a gate that cannot pass by looking nice: per-edge strain measured on the CPU. Layering ADDS 0.216 worst-case over the poses it blends, against locomotion's own 0.311 — blending distorts this skin less than the walk cycle already does. My predicted feathering fix was measured across 180 pose pairs and is monotonically the WRONG direction (0.198 hard cut → 0.228 fully feathered); the session corrected its own comment that had claimed otherwise from a box-rig render*
- [~] **16.4** **Reweight EVERY bending joint** (ADR 0015/0016/0020) — **owner priority** — *landed. **B-31 per motion: 44 edges over 100% strain -> 4**, and nothing else moved: hips still 0.999/0, knees 0.652/0, vertices at weight exactly 1.00 down 53 -> 5. The reweighting had to be made LOCAL twice, both caught by gates rather than inspection: global smoothing pushed the hips (sitting at 0.999, one thousandth under) to 1.177 and started them tearing, and a global leakage margin took a knee from 0.627 to 1.220 — so the band and the wide margin are both scoped to within 0.46 m of the shoulder joints, capped below the head so it cannot disturb 13.7's Face/Scalp split. **4 edges remain, both shoulders, and they are the floor** — margins 0.22-0.38 m and bands 0.10-0.34 m all land on the same four, and the clavicle does not move them. Recorded in the gate with a retirement condition, not asserted away. New hash `be7b3618f965eb0f`*
- [x] **16.6** Promote the per-edge strain instrument into the body contract's gate set as **§9.8** — *`every_bending_joint_survives_its_working_range` poses the body at fifteen joint cases and measures per-edge strain with the animation session's own metric. Every joint but the shoulder is asserted at zero outright; the shoulder is a **recorded B-31 violation, pinned** at its measured 32/26 edges so it cannot quietly worsen while the clavicle ruling is open, retiring together with the `!shoulder` exception. **A body that has not been posed has not been accepted — from now on it cannot be.** Amended per ADR 0020 to measure per MOTION: the shoulder case is arm 93 deg + chest 47 deg (scapulohumeral rhythm), the other fourteen are genuinely single-joint and unchanged.*
- [ ] ~~superseded by 16.4 above~~ — 53 of 195 `UpperArmR` vertices are bound at weight exactly 1.00 adjacent to vertices that are 49% `Spine`, and there is no clavicle, so one joint carries the whole rotation. Every use archetype needs 60–140° and all nine tear; the shipped walk never exceeds ~35°, which is why eight phases of walking never exposed it. **Acceptance: zero edges over 100% strain at 140° pitch**, >50% count reported. Contract-version event. **Preempts 13.9** — *owner: character asset pipeline*
- [x] **16.5** `skinMesh()` writes UVs — *one line, and it blocked every textured character: a CPU-skinned body left `Vertex::uv` at {0,0} and sampled ONE texel for its whole surface. Verified: the skinned body's UVs now span u 0.005..0.888, v 0.005..0.995 with **0 of 2097 vertices at (0,0)**, where previously all 2097 were. Unblocks the first face texture and therefore the retirement of ADR 0012's provisional Face waiver*
- [x] **14.7** Held items on the real body — *the absence is closed. `engine/*/character/held_items.*`: named attachment sockets (`hand_r`, `hand_l`, `back`, `hip_l`, `hip_r`) derived from the rig so they follow a variant's proportions with no per-variant data; `GripType` (one/two-handed/versatile) and a per-item grip transform declared in data; `placeHeldItem` transforms the item's static mesh into character-local space through `jointWorld[anchor] · socket · grip`. `buildPosedCharacter` now emits held items instead of skipping them — the line that dropped them is replaced and commented with why. A sword is an ordinary `.mgewear` row with `held true` naming a static `.mgemesh`, so a second weapon is a data change (13.12 applies here too); `tools/item_bake` produced the shipped `item_sword.mgemesh` once, offline. Held items deliberately never touch the fitting pipeline: they are rigid, have no `.mgefit`, and mask nothing. 10 tests in `tests/test_held_items.cpp`; capture `build/held_sword.ppm` from `tools/held_preview` (drawn and sheathed, front/side/back). **The render earned its place three times**: numbers said the sheathed sword was fine when it was lying flat across the shoulder blades, said it was fine again when its point stood above the character's head, and said the drawn grip was fine when the blade pointed horizontally forward like a presented lance. All three were one glance to see and invisible in a centroid. **Still open, and both across seams**: `GripType::TwoHanded` is parsed and carried but nothing yet occupies both hand slots — slot occupancy is gameplay's equipment machinery; and draw/sheath TRANSITION animations belong to the archetype library (`draw` exists there already, task 14.2)
...
- [x] **19.3** Use archetypes wired to the existing on-screen action button, so pressing it
  ...
  **Completed by ADR 0021:** the damage itself now lands on `strike` too, so the hand-rolled
  message delay this file used to carry is gone — there is nothing left to fake
