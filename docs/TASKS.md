...
- [~] **14.3** Phase-addressable timeline: wind-up / strike / recovery exposed as fractions, and the Phase 12 action model's damage moment hung on `strike` — *the animation half is done and now reachable from item data: `UsePhases` exposes the three as fractions plus a duration, `UsePlayer::strikeMoment()` gives the damage instant in seconds, and `update()` returns true on the single frame it is crossed — exactly once even when a long dt steps clean over it. The same `swing` lands at 0.233 s for a dagger and 0.729 s for a maul, neither number tuned.* **The remaining half's premise is wrong and should be corrected before anyone acts on it: there is no tuned delay to delete.** *This task, `CHARACTERS.md` §6.2 and ADR 0017 all describe replacing a hard-coded delay. `performUseHeld` applies damage **instantly**, on the input frame — `engine/src/framework/character.cpp:440`, `damage(victim, use->power)` inline. `useCooldown` is a rate limiter, not a damage timer, and no deferral mechanism exists anywhere (`damage()` has two call sites, both synchronous). So closing this is not a deletion — it means INTRODUCING deferral: somewhere to hold a pending strike per character (`CharacterComponent` field or a `CharacterSystem` side table, both `character.h`) and a behaviour change in `performUseHeld` that also changes what `ActionResult::target`/`amount` mean to the four areas that read them. ADR 0017's grant covers `items.h`, `use_archetype.h` and this area's own files and explicitly does not extend, so it is raised in `docs/status/animation.md` rather than taken* — **owner: gameplay mechanics, or a fresh ADR 0017-style assignment**
...
- [~] **16.4** **Reweight EVERY bending joint** (ADR 0015 + ADR 0016) — **owner priority** — *measured across all fifteen joint cases: **only the shoulders tear** (32 + 26 edges over 100%); elbows, knees, hips, ankles, wrists, neck, spine and chest are all at zero already, the hip closest at 0.999 worst. Reweighting is built and measured and **not landed**: it drives the strain constant K from 1.87 to 0.84 and the 1.00-weight cliff from 53 vertices to 5, but B-31 needs K < 0.53 and every configuration plateaus above it — band widths 6–45 cm, 4–20 rounds, tapered and hard prunes, dense and decimated meshes. The reason is geometric: a vertex at radius r displaces by 2r·sin(θ/2), so widening the band to R gives gradient 1/R and displacement ∝ R and the two cancel — **strain is scale-invariant in band width**, so no weighting reaches B-31 at 140° on one joint. What does work is two joints: measured at 140° total, splitting 70/70 across UpperArm and Chest (standing in for the clavicle) gives **1 edge over 100% on the reweighted body against 18 on one joint and 32 shipped**. Clavicle ruling requested with the numbers; reweighting held to ride that contract-version event per ADR 0011. Evidence: `docs/research/shoulder-reweight.md`*
