...
- [~] **8.10** Skinned mesh rendering + skeleton runtime (pose evaluation, GPU skinning) within Phase 2 budgets — *17-joint canonical rig + `evaluatePose` live; the template body is a real skinned mesh (36-byte `SkinVertex`, ≤4 influences) with a per-character 17-joint palette, and `skinMesh()` now applies the variant's morph deltas in bind space before the palette — the exact order the shader must use. The GPU half (palette + morph weights uploaded, skinned vertex shader) is the remaining work; tools skin on the CPU at load time meanwhile*
