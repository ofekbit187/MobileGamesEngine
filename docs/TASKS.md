...
- [ ] **13.7a** `B-9`'s authored hairline loop — **unblocked by ADR 0012**, which raises `B-5`'s LOD0 cap from 2 200 to 2 400 while leaving LOD1 (1 300) and LOD2 (650) untouched: the crowd draws LOD1/LOD2, so the budget P1 actually defends is unchanged, and 200 triangles on the near mesh buy the one boundary where hair, helmets and the bald fallback all meet. Carries the `body_mesh_has_human_proportions` fix with it (ADR 0012 Ruling 3) — that gate samples a fixed 60 mm world-space band and so fails for reasons unrelated to proportions; measure the torso's own width extent instead. Contract-version event — **owner: character asset pipeline**
- [ ] **13.7b** `stretch_above_max` waiver — per-asset data line in `assets/standards/skin_texture.mgestd` (asset, rule, measured value, reason, retirement condition), surfacing as a third verdict `WAIVED`, never `pass`; `stretch_max` itself stays 1.50. The seven non-face regions are waived outright; the **Face (2.09×) is waived provisionally with the owner's eye as its retirement condition** — the first authored face texture is the test, and if the distortion reads, a face-only re-unwrap rides a later contract-version event (ADR 0012 Ruling 1). **Owner: character asset pipeline, by explicit recorded seam exception** — textures-area files, no textures session live, and it gates the pipeline's own work
...
- [~] **13.10** The seven acceptance gates as tests (BODY_CONTRACT.md §9): mask integrity, +35 mm offset shell, hem-loop table, scalp-cap fallback, deterministic hash, groups & anchors, plus the existing gates — and the first authored garment set proving the contract end to end. **Gates are self-serve (P12): an artist imports and gets pass/fail with a reason — no engineer in the content loop** — *the self-serve half exists: `FitReport` returns a written pass/fail an artist can act on, and `tools/garment_fit` is the front door. Gate 5 (deterministic hash) is covered. Gates 1–4, 6 and the first authored garment set wait on the body deliverables 13.6–13.9*
